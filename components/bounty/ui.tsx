...
export function Card({ children, pad = 24, tint = T.surface, style = {}, onClick }: { 
  children: ReactNode, 
  pad?: number, 
  tint?: string, 
...
export function Btn({ children, onClick, tone = 'primary', size = 'md', icon, disabled, full, style }: {
  ...
  tone?: 'primary' | 'secondary' | 'danger' | 'emerald' | 'blue' | 'amber' | 'ghost',
  size?: 'sm' | 'md' | 'lg', icon?: ReactNode, disabled?: boolean, full?: boolean, 
  style?: React.CSSProperties
}) {
  const bg = {
    primary: T.ink,
